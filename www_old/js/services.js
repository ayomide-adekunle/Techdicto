...
.service('BookMarkService', function (_, $rootScope){
	...
	this.bookmarkWord = function(bookmark_word){
		//console.log(bookmark_word);

		var user_bookmarks = !_.isUndefined(window.localStorage.ionFullApp_feed_words) ?
														JSON.parse(window.localStorage.ionFullApp_feed_words) : [];

		//check if this post is already saved
		console.log(user_bookmarks);

		var existing_post = _.find(user_bookmarks, function(word){ return word.id == bookmark_word.id; });


		if(!existing_post){
			user_bookmarks.push({
				id: bookmark_word.id,
				keyword : bookmark_word.keyword,
				meaning:  bookmark_word.meaning,
				details:  bookmark_word.details
			});
		}
        console.log(user_bookmarks);
		window.localStorage.ionFullApp_feed_words = JSON.stringify(user_bookmarks);
